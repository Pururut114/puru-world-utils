...
| `_Smoothness` | Общая glossiness, один слайдер на все 4 слоя (не per-layer) |
...
- `_Smoothness`/`_NormalStrength` общие на весь материал, не per-layer — осознанное упрощение ("простенький шейдерок"), если понадобится per-layer — расширять через доп. каналы маски или отдельные свойства.
